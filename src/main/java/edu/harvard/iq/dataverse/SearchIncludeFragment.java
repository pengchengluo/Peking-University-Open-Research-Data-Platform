...
import cn.edu.pku.lib.dataverse.util.DataverseLocale;
...
//    private Map<String, String> friendlyName = new HashMap<>();
    ...
    public void search(boolean onlyDataRelatedToMe) {
        ...
        boolean isZh = dataverseLocale.isLocaleZh();
...
//        int count = 0;
        ...
        try {
             ...
             */
            int numRows = 10;
            solrQueryResponse = searchService.search(session.getUser(), dataverse, queryToPassToSolr, filterQueriesFinal, 
                    sortField, sortOrder, paginationStart, onlyDataRelatedToMe, numRows, dataverseLocale.getLocale());
            solrQueryResponseAllTypes = searchService.search(session.getUser(), dataverse, queryToPassToSolr,
                    filterQueriesFinalAllTypes, sortField, sortOrder, paginationStart, onlyDataRelatedToMe, numRows, dataverseLocale.getLocale());
            ...
            for (SolrSearchResult solrSearchResult : searchResults) {
                ...
                if (solrSearchResult.getType().equals("dataverses")) {
                    ...
                    if (dataverseInCard != null) {
                        if(isZh)
                            solrSearchResult.setDataverseAffiliation(dataverseInCard.getAffiliationZh());
                        else 
