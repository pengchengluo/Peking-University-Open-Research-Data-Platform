...
public class LoginPage implements java.io.Serializable {
    ...
    public String login() {
        ...
        try {
            ...
            String redirectDataset = null;
            String redirectDataverse = null;
            for(Cookie cookie : cookies){
                if(cookie.getName().equals("redirectDatasetId")){
                    cookie.setMaxAge(0);
                    response.addCookie(cookie); 
                    redirectDataset = "dataset.xhtml?persistentId=doi:"+cookie.getValue()+"&faces-redirect=true";
                }else if(cookie.getName().equals("redirectDataverseId")){
                    cookie.setMaxAge(0);
                    response.addCookie(cookie); 
                    redirectDataverse = "dataverse.xhtml?alias="+cookie.getValue()+"&faces-redirect=true";
                }
            }
            if(redirectDataset != null){
                return redirectDataset;
            }else if(redirectDataverse != null){
                return redirectDataverse;
            }
