...
import cn.edu.pku.lib.dataverse.util.SearchResultToLocale;
...
public class SearchServiceBean {
    ...
    @EJB
    SearchResultToLocale searchResultToLocale;
    ...
    public SolrQueryResponse search(User user, Dataverse dataverse, String query, List<String> filterQueries, String sortField, String sortOrder, int paginationStart, boolean onlyDatatRelatedToMe, int numResultsPerPage) throws SearchException {
        return search(user, dataverse, query, filterQueries, sortField, sortOrder, paginationStart, onlyDatatRelatedToMe, numResultsPerPage, Locale.ENGLISH);
    }
    
    public SolrQueryResponse search(User user, Dataverse dataverse, String query, List<String> filterQueries, String sortField, String sortOrder, int paginationStart, boolean onlyDatatRelatedToMe, int numResultsPerPage, Locale locale) throws SearchException {
...
//        solrQuery.setSort(sortClause);
        ...
        if(isZh){
            solrFieldsToHightlightOnMap.put(SearchFields.NAME_ZH, "名称");
            solrFieldsToHightlightOnMap.put(SearchFields.AFFILIATION_ZH, "单位");
            solrFieldsToHightlightOnMap.put(SearchFields.DESCRIPTION_ZH, "描述");
        }else{
...
//        solrQuery.addFacetField(SearchFields.AUTHOR_STRING);
        ...
        if(isZh){
            solrQuery.addFacetField(SearchFields.AFFILIATION_ZH);
        }else{
...
//        solrQuery.addNumericRangeFacet(SearchFields.DISTRIBUTION_DATE_YEAR_ONLY, citationYearRangeStart, citationYearRangeEnd, citationYearRangeSpan);
        ...
        try {
            ...
            DatasetFieldType titleDatasetFieldZh = datasetFieldService.findByName(DatasetFieldConstant.titleZh);
            titleSolrField = titleDatasetField.getSolrField().getNameSearchable();
            titleZhSolrField = titleDatasetFieldZh.getSolrField().getNameSearchable();
        ...
        while (iter.hasNext()) {
            ...
            String citationZh = (String) solrDocument.getFieldValue(SearchFields.DATASET_CITATION_ZH);
            String persistentUrl = (String) solrDocument.getFieldValue(SearchFields.PERSISTENT_URL);
            
            String name = "";
            if (type.equals("dataverses")) {
                name = isZh ? (String) solrDocument.getFieldValue(SearchFields.NAME_ZH):
                    (String) solrDocument.getFieldValue(SearchFields.NAME);
...
//            ArrayList titles = (ArrayList) solrDocument.getFieldValues(SearchFields.TITLE);
            ...
            String titleZh = (String) solrDocument.getFieldValue(titleZhSolrField);
...
//            logger.info("title: " + title);
            ...
            String dateToDisplayOnCardZh = (String) solrDocument.getFirstValue(SearchFields.RELEASE_OR_CREATE_DATE_SEARCHABLE_TEXT_ZH);
                ...
                for (Map.Entry<String, String> entry : solrFieldsToHightlightOnMap.entrySet()) {
                    ...
                    if (highlightSnippets != null) {
                        ...
                        SolrField solrField = new SolrField(field, SolrField.SolrType.STRING, true, true,
                                solrFieldsToHightlightOnMap.get(field));
...
//            logger.info(id + ": " + description);
            ...
            solrSearchResult.setDateToDisplayOnCardZh(dateToDisplayOnCardZh);
            ...
            String description = isZh ? (String) solrDocument.getFieldValue(SearchFields.DESCRIPTION_ZH) : 
                    (String) solrDocument.getFieldValue(SearchFields.DESCRIPTION);
...
//                solrSearchResult.setApiUrl(baseUrl + "/api/dataverses/" + entityid);
                 ...
                 */
                ArrayList<String> datasetDescriptions;
                if(isZh)
                    datasetDescriptions = (ArrayList<String>) solrDocument.getFieldValue(SearchFields.DATASET_DESCRIPTION_ZH);
                else
                    datasetDescriptions = (ArrayList<String>) solrDocument.getFieldValue(SearchFields.DATASET_DESCRIPTION);
                ...
                solrSearchResult.setCitationZh(citationZh);
                if (title != null) {
//                    solrSearchResult.setTitle((String) titles.get(0));
                ...
                if(titleZh != null){
                    solrSearchResult.setTitleZh(titleZh);
                } else {
                    logger.info("No title_zh indexed. Setting to empty string to prevent NPE. Dataset id " + entityid + " and version id " + datasetVersionId);
                    solrSearchResult.setTitleZh("");
                }
...
//                solrSearchResult.setApiUrl(baseUrl + "/api/meta/datafile/" + entityid);
            ...
            if(isZh){
                parent.put("name", (String) solrDocument.getFieldValue(SearchFields.PARENT_NAME_ZH));
            }else{
...
//                    logger.info("adding <<<" + staticSearchField + ":" + friendlyName + ">>>");
        ...
        searchResultToLocale.toLocale(solrQueryResponse, locale);
