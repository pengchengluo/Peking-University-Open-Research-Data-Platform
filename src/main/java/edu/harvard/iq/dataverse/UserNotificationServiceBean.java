...
public class UserNotificationServiceBean {
    ...
    public void deleteByUserId(Long userId){
        Query query = em.createQuery("delete from UserNotification as o where o.user.id="+userId);
        query.executeUpdate();
    }
    
    public void sendNotification(AuthenticatedUser dataverseUser, Timestamp sendDate, 
            Type type, Long objectId) {
        sendNotification(dataverseUser, sendDate, type, objectId, Locale.ENGLISH);
    }
    
    public void sendNotification(AuthenticatedUser dataverseUser, Timestamp sendDate,
            Type type, Long objectId, Locale locale){
        sendNotification(dataverseUser, sendDate, type, objectId, null, locale);
    }
    
    public void sendNotification(AuthenticatedUser dataverseUser, Timestamp sendDate,
            Type type, Long objectId, String message ,Locale locale) {
        ...
        userNotification = save(userNotification);
        if (mailService.sendNotificationEmail(userNotification, locale)){
