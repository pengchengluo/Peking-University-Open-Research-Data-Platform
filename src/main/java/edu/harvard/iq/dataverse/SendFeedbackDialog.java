...
public class SendFeedbackDialog implements java.io.Serializable {
    ...
    private String defaultRecipientEmail = "zhul@lib.pku.edu.cn";
    ...
    public String getMessageTo() {
        String email = "";
        if(recipient == null)
            email = defaultRecipientEmail;//JH.localize("contact.support");
        else{
...
//            return JH.localize("dataset") + " " + JH.localize("contact.contact");
  ...
  public void validateUserEmail(FacesContext context, UIComponent component, Object value) throws ValidatorException {
    ...
    public String sendMessage(Locale locale) {
        String email = "";
        if (recipient!=null) {
            if (recipient.isInstanceofDataverse() ) {
               email = getDataverseEmail((Dataverse)recipient);
        ...
        if (isLoggedIn() && userMessage!=null) {
            mailService.sendMail(loggedInUserEmail(), email, getMessageSubject(), userMessage, locale);
            userMessage = "";
            return null;
        } else {
            if (userEmail != null && userMessage != null) {
                mailService.sendMail(userEmail, email, getMessageSubject(), userMessage, locale);
                userMessage = "";
                return null;
            } else {
                userMessage = "";
