...
public class ManagePermissionsPage implements java.io.Serializable {
    ...
    public void removeGroup(){
        if(selectedGroup instanceof ExplicitGroup){
            if(roleAssigneeService.getAssignmentsFor(selectedGroup.getIdentifier()).size()>0){
                JH.addMessage(FacesMessage.SEVERITY_WARN, "The explicit group was not able to be removed. Please remove all roles assigned to this group.");
            }else{
                try{
                    commandEngine.submit(new DeleteExplicitGroupCommand(
                            session.getUser(), (ExplicitGroup)selectedGroup));
                    JsfHelper.addSuccessMessage("Group "+selectedGroup.getDisplayName()+ " ("+selectedGroup.getDisplayName()+") was removed.");
                }catch(PermissionException ex){
                    JH.addMessage(FacesMessage.SEVERITY_ERROR, "The explicit group was not able to be removed.",
                            "Permissions " + ex.getRequiredPermissions().toString() + " missing.");
                } catch (CommandException ex) {
                    JH.addMessage(FacesMessage.SEVERITY_FATAL, "The explicit group could not be removed.");
                    logger.log(Level.SEVERE, "Error removing explicit group: " + ex.getMessage(), ex);
                }
            }
        }
        showGroupMessages();
    ...
    public List<Entry<Group,Boolean>> getGroups(){
        Map<Group, Boolean> groupMap = new LinkedHashMap<>();
        groupMap.putAll(explicitGroupSvc.findAvailableFor(dvObject, session.getUser()));
        for ( Group g : groupService.findGlobalGroups() ) {
            groupMap.put(g, false);
        }
        List<Entry<Group, Boolean>> entries = new ArrayList<>();
        for(Entry<Group,Boolean> entry : groupMap.entrySet()){
            entries.add(entry);
        }
        return entries;
    }
    ...
    */
    ...
    UserType explicitGroupRequestJoinUserType;
    ...
    public void initExplicitGroupDialog(ActionEvent ae) {
        ...
        this.setExplicitGroupRequestJoinUserType(UserType.ORDINARY);
    ...
    public void saveExplicitGroup(ActionEvent ae) {
        ...
        eg.setRequestJoinUserType(getExplicitGroupRequestJoinUserType());
    ...
    public UserType getExplicitGroupRequestJoinUserType() {
        return explicitGroupRequestJoinUserType;
    }

    public void setExplicitGroupRequestJoinUserType(UserType explicitGroupRequestJoinUserType) {
        this.explicitGroupRequestJoinUserType = explicitGroupRequestJoinUserType;
    }
    ...
    */
    ...
    boolean renderGroupMessages = false;
    ...
    public boolean getRenderGroupMessages() {
        return renderGroupMessages;
    }

    public void setRenderGroupMessages(boolean renderGroupMessages) {
        this.renderGroupMessages = renderGroupMessages;
    }
