...
//    }
    public String init() {        
        if (dataverse.getAlias() != null || dataverse.getId() != null || ownerId == null) {// view mode for a dataverse
            ...
            eventLog.setDataverseId(dataverse.getId());
    ...
    public List<ExplicitGroup> getExplicitGroups(){
        Set<ExplicitGroup> groupSet = explicitGroupSvc.findAvailableFor(dataverse);
        List<ExplicitGroup> groups = new ArrayList();
        for(ExplicitGroup group : groupSet){
            groups.add(group);
        }
        return groups;
    }
    ...
    public void requestJoinGroup(ExplicitGroup group){
        if(group.getRequestJoinUserType()==UserType.ADVANCE && 
                ((AuthenticatedUser)session.getUser()).getUserType()==UserType.ORDINARY){
        ...
        explicitGroupSvc.persist(group);
        ...
        usageLogIndexService.index(eventLog);
        for (AuthenticatedUser au : permissionService.getUsersWithPermissionOn(Permission.ManageDataversePermissions, dataverse)) {
            userNotificationService.sendNotification(au, new Timestamp(new Date().getTime()),
                    UserNotification.Type.REQUESTJOINGROUP, group.getId(),FacesContext.getCurrentInstance().getViewRoot().getLocale());
        }
    }
}
