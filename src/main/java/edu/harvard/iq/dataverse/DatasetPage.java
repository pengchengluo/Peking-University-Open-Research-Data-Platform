...
//    @EJB
//    ElasticSearchLoggerServiceBean esLoggerServiceBean;
    @Inject
    ...
    private String displayCitationZh;
    ...
    public String getDisplayCitationZh() {
        return displayCitationZh;
    }

    public void setDisplayCitationZh(String displayCitationZh) {
        this.displayCitationZh = displayCitationZh;
    }
    ...
    public String init() {
        ...
        if (dataset.getId() != null || versionId != null || persistentId != null) { // view mode for a dataset     
           ...
           // If this DatasetVersion is unpublished and permission is doesn't have permissions:
            ...
            displayCitationZh = dataset.getCitation(true, workingVersion, "zh");
            ...
            EventLog eventLog = usageLogIndexService.buildEventLog(EventLog.EventType.VIEW_DATASET,
                    session.getUser(),
                    (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest());
            eventLog.setDatasetId(dataset.getId());
    ...
    private void callDownloadServlet(String downloadType, Long fileId) {
        EventLog eventLog = usageLogIndexService.buildEventLog(EventLog.EventType.DOWNLOAD_FILE,
                    session.getUser(),
                    (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest());
        eventLog.setDatafileId(fileId);
        usageLogIndexService.index(eventLog);
        ...
        if (session.getUser().isAuthenticated()) {
            ...
            for (DatasetField dsf : dataset.getEditVersion().getDatasetFields()) {
                if ((dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.depositor)
                        || dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.depositorZh)) && dsf.isEmpty()) {
                ...
                if ((dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.dateOfDeposit)
                        || dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.dateOfDepositZh)) && dsf.isEmpty()) {
                ...
                if ((dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContact)
                        || dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContactZh)) && dsf.isEmpty()) {
                    for (DatasetFieldCompoundValue contactValue : dsf.getDatasetFieldCompoundValues()) {
                        for (DatasetField subField : contactValue.getChildDatasetFields()) {
                            if (subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContactName)
                                    || subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContactNameZh)) {
                            ...
                            if (subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContactAffiliation)
                                    || subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContactAffiliationZh)) {
                            ...
                            if (subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContactEmail)
                                    || subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.datasetContactEmailZh)) {
                ...
                if ((dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.author)
                        || dsf.getDatasetFieldType().getName().equals(DatasetFieldConstant.authorZh)) && dsf.isEmpty()) {
                    for (DatasetFieldCompoundValue authorValue : dsf.getDatasetFieldCompoundValues()) {
                        for (DatasetField subField : authorValue.getChildDatasetFields()) {
                            if (subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.authorName)
                                    || subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.authorNameZh)) {
                                subField.getDatasetFieldValues().get(0).setValue(au.getLastName() + ", " + au.getFirstName());
                            }
                            if (subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.authorAffiliation)
                                    || subField.getDatasetFieldType().getName().equals(DatasetFieldConstant.authorAffiliationZh)) {
                                subField.getDatasetFieldValues().get(0).setValue(au.getAffiliation());
                            }
                        }
                    }
    ...
    public String getSelectedFilesIdsString() {
        ...
        for (FileMetadata fmd : this.selectedFiles) {
            ...
            EventLog eventLog = usageLogIndexService.buildEventLog(EventLog.EventType.DOWNLOAD_FILE,
                    session.getUser(),
                    (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest());
            eventLog.setDatafileId(fmd.getDataFile().getId());
            usageLogIndexService.index(eventLog);
        }
...
//delete for files that have been injested....
    ...
    public void requestAccess(DataFile file) {
        EventLog eventLog = usageLogIndexService.buildEventLog(EventLog.EventType.REQUEST_ACCESS_FILE,
                    session.getUser(),
                    (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest());
        eventLog.setDatafileId(file.getId());
        usageLogIndexService.index(eventLog);
    ...
    public List<Entry<ExplicitGroup, List<FileMetadata>>> getDataFileFileDownloadGroup() {
        ...
        for (FileMetadata fileMetadata : this.workingVersion.getFileMetadatas()) {
            ...
            if (file.isRestricted() || file.getFileMetadata().isRestricted()) {
                ...
                for (RoleAssignment ra : ras) {
                    ...
                    String identifier = ra.getAssigneeIdentifier();
                    if (identifier.startsWith("&explicit")
                            && ra.getRole().permissions().contains(Permission.DownloadFile)) {
                        ExplicitGroup g = explicitGroupService.findByAlias(identifier.substring(identifier.indexOf('/') + 1));
                        List<FileMetadata> fileMetadatas = downloadGroup.get(g);
                        if (fileMetadatas == null) {
                            fileMetadatas = new ArrayList<>();
                            downloadGroup.put(g, fileMetadatas);
                        }
        ...
        List<Entry<ExplicitGroup, List<FileMetadata>>> entries = new ArrayList<>();
        for (Entry<ExplicitGroup, List<FileMetadata>> entry : downloadGroup.entrySet()) {
            entries.add(entry);
        }
        Collections.sort(entries, new Comparator<Entry<ExplicitGroup, List<FileMetadata>>>(){
            @Override
            public int compare(Entry<ExplicitGroup, List<FileMetadata>> entry1,
                    Entry<ExplicitGroup, List<FileMetadata>> entry2) {
